...
import weatherReport.API;
import weatherReport.transport.data.WeatherDTO;
import weatherReport.transport.data.ForecastDTO;
...
public class OpenWeatherService {
    ...
    private static final String CurrentEndpoint = "/data/2.5/weather?lat=%f&lon=%f&units=metric&lang=ru&appid=" + API.KEY;
    // Эндпоинт для получения прогноза погоды
    // %f используются для подстановки значений широты и долготы
    private static final String ForecastEndpoint = "/data/2.5/forecast?lat=%f&lon=%f&units=metric&lang=ru&appid=" + API.KEY;
    // Эндпоинт для геокодирования (получения координат по названию города)
    // %s используется для подстановки названия города
    private static final String GeocodingEndpoint = "/geo/1.0/direct?q=%s,,RU&limit=5&appid=" + API.KEY;
    /*
    * Получает список GeocodingDTO (объектов с информацией о координатах) по названию города
    ...
    public static ArrayList<GeocodingDTO> getGeocoding(String name) throws NetException {
        try {
            ...
            logger.info("Request sent: " + request.uri().toString());
            ...
            throw new NetException("api use error; check 'logs/errors.log'");
        }
    }
